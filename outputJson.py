...
for iii in xrange(0, 1):
    try:
        res = rs.get(url.format(limit, offset), headers=head)
        ...
        for i in r:
            ...
            try:
                ...
                response.append(i)
                print '%d users in the house' % (offset+len(response))
            except:
                print "no response"
        user['user'] = response
        ...
        blogInfo['blog'] = blogs
        try:
            with open('/Workspace/data/user/ifoodUsers_%d.json' % (offset+limit), 'w') as f:
                json.dump(user, f)
            with open('/Workspace/data/restaurant/ifoodRestaurant_%d.json' % (offset+limit), 'w') as f:
                json.dump(restat, f)
            with open('/Workspace/data/blog/ifoodBlog_%d.json' % (offset+limit), 'w') as f:
